...
export default function GradeStep() {
  const { dispatch } = useSellForm();

  const handleCertificationGenerated = (certificationId: string) => {
    dispatch({
      type: 'UPDATE_DATA',
      payload: {
        certificationId,
  ...
  return (
    <StepLayout onNext={onNext} onPrev={onPrev} showPrevButton={true}>
      ...
      <CertificationGenerator onCertificationGenerated={handleCertificationGenerated} />
    </StepLayout>
  );
}
